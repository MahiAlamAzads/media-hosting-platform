...
BACKUP_DIR="$TARGET/.phase-backups/phase2-types-final-fix-$STAMP"
...
for required in \
  "apps/api/src/modules/folders/folders.ts" \
  "apps/api/src/modules/media/media.ts" \
  "apps/api/src/modules/uploads/uploads.ts"
do
  if [[ ! -f "$TARGET/$required" ]]; then
    echo "Missing expected Phase 2 file: $TARGET/$required" >&2
    exit 66
  fi
done
...
echo
echo "Phase 2 TypeScript/Prisma fix applied."
