...
router.post("/", asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new AppError(413, "STORAGE_QUOTA_EXCEEDED", "Workspace storage quota exceeded.");
  }
  ...
  const result = await prisma.$transaction(async tx => {
    const asset = await tx.mediaAsset.create({
      data: {
        ...
        storageKey: `tenants/${auth.workspaceId}/originals/pending-${randomUUID()}`,
    ...
    return { assetId: asset.id, session };
  });
...
router.get("/:uploadId", asyncHandler(async (req, res) => {
  ...
  const session = await prisma.uploadSession.findFirst({
    ...
    include: {
      chunks: {
        ...
        select: { chunkIndex: true, sizeBytes: true }
      }
    }
  });

  if (!session) {
    throw new AppError(404, "UPLOAD_NOT_FOUND", "Upload session was not found.");
  }
  ...
  res.json({
    data: {
      ...
      uploadedChunkIndexes: session.chunks.map(chunk => chunk.chunkIndex),
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    const auth = req.auth!;
    const chunkIndex = z.coerce.number().int().nonnegative().parse(req.params.chunkIndex);
    const body = req.body;
    ...
    if (!session || session.status !== "ACTIVE") {
      throw new AppError(404, "UPLOAD_NOT_ACTIVE", "Upload session is not active.");
    }
    ...
    if (chunkIndex >= session.expectedChunks) {
      throw new AppError(400, "INVALID_CHUNK_INDEX", "Chunk index is outside the expected range.");
    }
...
router.post("/:uploadId/complete", asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new AppError(409, "UPLOAD_INCOMPLETE", "Not all upload chunks have been received.");
  }

  const expectedIndexes = Array.from(
    { length: session.expectedChunks },
    (_, index) => index
  );

  if (
    session.chunks.some(
      (chunk, index) => chunk.chunkIndex !== expectedIndexes[index]
    )
  ) {
    throw new AppError(409, "UPLOAD_CHUNK_GAP", "One or more upload chunks are missing.");
  }
  ...
  try {
    await concatenateStorageFiles(
      session.chunks.map(chunk => chunk.storageKey),
      session.mediaAsset.storageKey
    );

    const actualBytes = await storageFileSize(session.mediaAsset.storageKey);
    if (actualBytes !== session.expectedBytes) {
      throw new AppError(409, "UPLOAD_SIZE_MISMATCH", "Completed file size does not match.");
    }

    const actualChecksum = await storageFileChecksum(session.mediaAsset.storageKey);
    if (
      session.mediaAsset.checksumSha256 &&
      actualChecksum !== session.mediaAsset.checksumSha256
    ) {
      throw new AppError(409, "UPLOAD_CHECKSUM_MISMATCH", "Completed file checksum does not match.");
    }
    ...
    await prisma.$transaction([
      ...
      prisma.workspace.update({
        where: { id: auth.workspaceId },
        data: {
          storageReservedBytes: { decrement: session.expectedBytes },
          storageUsedBytes: { increment: session.expectedBytes }
        }
      })
    ]);

    await Promise.all(
      session.chunks.map(chunk => removeStorageFile(chunk.storageKey))
    );

    res.json({
      data: {
        assetId: session.mediaAssetId,
        status: "READY",
        checksumSha256: actualChecksum,
