...
describe("Phase 13 PAYG contracts", () => {
  it("mounts the authenticated PAYG router", () => {
    expect(appSource).toContain('app.use("/api/v1/billing", paygRouter)');
  });

  it("mounts the raw Stripe webhook before JSON parsing", () => {
    const webhook = appSource.indexOf(
