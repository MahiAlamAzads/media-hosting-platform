...
async function processSuccessfulNotification(payload: Record<string, string>) {
  ...
  if (event.processedAt && (attempt.status === "PAID" || attempt.invoice.status === "PAID")) {
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      await prisma.paymentWebhookEvent.update({
        where: { id: event.id },
        data: {
          processingError: error instanceof Error ? error.message : "IPN processing failed."
        }
      });
      throw error;
    }
...
router.all(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.redirect(303, redirectUrl(invoiceId ?? payload.value_a ?? null, "failed"));
  })
);
...
router.all(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.redirect(303, redirectUrl(invoiceId ?? payload.value_a ?? null, "cancelled"));
  })
);
