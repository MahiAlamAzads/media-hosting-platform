...
describe("Phase 21 Redis performance contracts", () => {
  ...
  it("caches public descriptors and invalidates mutations", () => {
    const publicRoute = source("src/modules/public/public-media.route.ts");
    const media = source("src/modules/media/media.ts");
    const uploads = source("src/modules/uploads/uploads.ts");
