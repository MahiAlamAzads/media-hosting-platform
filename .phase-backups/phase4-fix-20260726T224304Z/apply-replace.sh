...
BACKUP_DIR="$TARGET/.phase-backups/phase4-$STAMP"
...
if [[ -f "$TARGET/.env" ]] && ! grep -q '^API_KEY_PEPPER=' "$TARGET/.env"; then
  API_KEY_PEPPER="$(openssl rand -base64 48 | tr -d '\n')"
  {
    echo
    echo "API_KEY_PEPPER=$API_KEY_PEPPER"
  } >> "$TARGET/.env"
  echo "Generated API_KEY_PEPPER in existing .env."
fi

chmod +x "$TARGET/apply-replace.sh"

echo
echo "Media Hosting Phase 4 applied."
echo "Backup: $BACKUP_DIR"
echo
echo "Preserved: .env values, migrations, storage, PostgreSQL data"
echo
echo "Run:"
echo "  cd \"$TARGET\""
echo "  pnpm install"
