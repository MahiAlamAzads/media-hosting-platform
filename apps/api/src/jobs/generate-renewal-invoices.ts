...
const now = new Date();
const leadEnd = addDays(
  now,
  env.PAYMENT_RENEWAL_LEAD_DAYS
);
...
for (const subscription of subscriptions) {
  ...
  const targetVersion =
    approvedChange?.requestedPlanVersion ??
    subscription.planVersion;
  const currency =
    approvedChange?.currency ?? subscription.currency;
  const term =
    approvedChange?.subscriptionTerm ??
    subscription.subscriptionTerm;
  const interval = subscriptionTermToInterval(term);

  const offer = targetVersion.offers.find(item =>
  ...
  const ownerId =
    subscription.workspace.members[0]?.user.id;
  if (!ownerId) {
