...
import { isPaygEligibleMetric, paygEligibleMetrics } from "./payg.service.js";
import { chargePendingPaygForWorkspace } from "./payg-charge.service.js";
      ...
      subscription.currency === "BDT"
        ? {
            monthlySpendCapMinor: BigInt(
              env.PAYG_DEFAULT_MONTHLY_CAP_BDT_MINOR,
            ),
            chargeThresholdMinor: BigInt(
              env.PAYG_DEFAULT_CHARGE_THRESHOLD_BDT_MINOR,
            ),
          }
        : {
            monthlySpendCapMinor: BigInt(
              env.PAYG_DEFAULT_MONTHLY_CAP_USD_MINOR,
            ),
            chargeThresholdMinor: BigInt(
              env.PAYG_DEFAULT_CHARGE_THRESHOLD_USD_MINOR,
            ),
          };
      ...
      data: {
        ...
        policy: policy
          ? {
              ...
              monthlySpendCapMinor: policy.monthlySpendCapMinor.toString(),
              chargeThresholdMinor: policy.chargeThresholdMinor.toString(),
              defaultPaymentMethodId: policy.defaultPaymentMethodId,
          ...
          : {
              status: "DISABLED",
              currency: subscription.currency,
              monthlySpendCapMinor: defaults.monthlySpendCapMinor.toString(),
              chargeThresholdMinor: defaults.chargeThresholdMinor.toString(),
        ...
        currentSpendMinor: (spend._sum.amountMinor ?? 0n).toString(),
        ...
        metrics: subscription.planVersion.entitlements.map((entitlement) => ({
          metric: entitlement.metric,
          includedAmount: entitlement.includedAmount.toString(),
          overageUnit: entitlement.overageUnit?.toString() ?? null,
          overagePriceMinor:
            subscription.currency === "BDT"
    ...
    const input = z
      .object({
        provider: z
          ...
          .default(env.PAYG_CARD_PROVIDER),
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    requireBillingManager(req.auth!.role);
    const paymentMethodId = z.string().cuid().parse(req.params.paymentMethodId);

    await prisma.$transaction((tx) =>
      setDefaultPaymentMethod(tx, req.auth!.workspaceId, paymentMethodId),
    );
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (method.provider === "STRIPE") {
      await detachStripePaymentMethod(method.providerPaymentMethodId);
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    const input = z
      .object({
        ...
        defaultPaymentMethodId: z.string().cuid().nullable(),
    ...
    if (input.chargeThresholdMinor > input.monthlySpendCapMinor) {
      throw new AppError(
        422,
    ...
    const entitlementMap = new Map(
      subscription.planVersion.entitlements.map((item) => [item.metric, item]),
    );
    ...
    for (const item of enabledMetrics) {
      ...
      if (!entitlement || !entitlement.overageUnit || !price) {
        throw new AppError(
          422,
    ...
    const policy = await prisma.$transaction(async (tx) => {
      const saved = await tx.paygPolicy.upsert({
        ...
        create: {
          ...
          monthlySpendCapMinor: input.monthlySpendCapMinor,
          chargeThresholdMinor: input.chargeThresholdMinor,
      ...
      await tx.auditLog.create({
        data: {
          ...
          metadata: {
            currency: input.currency,
            monthlySpendCapMinor: input.monthlySpendCapMinor.toString(),
            chargeThresholdMinor: input.chargeThresholdMinor.toString(),
            enabledMetrics: enabledMetrics.map((item) => item.metric),
          },
        },
      });
    ...
    await chargePendingPaygForWorkspace(req.auth!.workspaceId, true);
