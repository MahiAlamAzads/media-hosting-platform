...
import type { EntitlementValue, UsageMetricName } from "./billing.types.js";
import { ensurePaygCoverageInTransaction } from "./payg.service.js";
...
async function assertPaygAwareLimit(
  ...
  await ensurePaygCoverageInTransaction(tx, {
    ...
    limit: input.limitOverride ?? input.entitlement.includedAmount,
...
export async function assertUploadAllowedInTransaction(
  ...
  const maxFile = requireEntitlement(billing.values, "MAX_FILE_SIZE_BYTES");
  ...
  const storage = requireEntitlement(billing.values, "STORAGE_BYTES");
  ...
  await assertPaygAwareLimit(tx, {
    ...
    current: workspace.storageUsedBytes + workspace.storageReservedBytes,
    requested: input.expectedBytes,
    entitlement: storage,
  ...
  const assets = requireEntitlement(billing.values, "ACTIVE_ASSETS");
  ...
  const upload = requireEntitlement(billing.values, "UPLOAD_BYTES");
...
export async function assertMeteredUsageAllowed(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const billing = await loadEntitlementsInTransaction(tx, workspaceId);
    const entitlement = requireEntitlement(billing.values, metric);
