...
import { enforceLoginThrottle, normalizeEmail, recordLoginAttempt } from "../../shared/auth-policy.js";
...
router.post("/register", asyncHandler(async (req, res) => {
  ...
  if (existing) {
    res.status(202).json({
      data: { message: "If this email can be registered, verification instructions will be sent." },
...
router.post("/resend-verification", asyncHandler(async (req, res) => {
  ...
  res.json({
    data: { message: "If verification is required, a new email will be sent." },
    meta: { requestId: req.id }
  });
}));
...
router.post("/login", asyncHandler(async (req, res) => {
  ...
  if (!membership) throw new AppError(403, "NO_WORKSPACE", "No active workspace membership found.");
