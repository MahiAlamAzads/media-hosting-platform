...
router.post("/", asyncHandler(async (req, res) => {
  ...
  if (input.folderId) {
    const folder = await prisma.folder.findFirst({
      where: { id: input.folderId, workspaceId: auth.workspaceId },
      select: { id: true }
    });
  ...
  const result = await prisma.$transaction(async tx => {
    ...
    await tx.workspace.update({
      where: { id: auth.workspaceId },
      data: { storageReservedBytes: { increment: expectedBytes } }
    });
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existing) {
      res.json({
        data: { accepted: true, duplicate: true, chunkIndex },
        meta: { requestId: req.id }
      });
      return;
    }
...
router.post("/:uploadId/complete", asyncHandler(async (req, res) => {
  ...
  const mediaAsset = await prisma.mediaAsset.findFirst({
    where: { id: session.mediaAssetId, workspaceId: auth.workspaceId }
  });
...
router.delete("/:uploadId", asyncHandler(async (req, res) => {
  ...
  await prisma.$transaction([
    ...
    prisma.workspace.update({
      where: { id: auth.workspaceId },
      data: { storageReservedBytes: { decrement: session.expectedBytes } }
    }),
    prisma.mediaAsset.update({
