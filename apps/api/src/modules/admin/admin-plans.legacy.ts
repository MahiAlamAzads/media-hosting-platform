    ...
    const input = z
      .object({
        ...
        prices: z
          ...
          .length(4),
        entitlements: z.array(entitlementSchema).length(usageMetrics.length),
      })
      .superRefine((value, context) => {
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      const price = (
        ...
        const item = input.prices.find(
          (value) => value.currency === currency && value.interval === interval,
        );
        if (!item) {
          throw new AppError(
            422,
      ...
      .object({
        offers: z
          .array(
            z.object({
              currency: z.enum(["BDT", "USD"]),
              term: z.enum(["FREE", "THREE_MONTHS", "SIX_MONTHS", "ONE_YEAR"]),
              amountMinor: moneySchema,
              isPublic: z.boolean().default(true),
              isActive: z.boolean().default(true),
            }),
      ...
      data: subscriptions.map((item) => ({
        ...
        pendingChange: item.workspace.subscriptionChanges[0]
          ? {
              ...
              planCode:
                item.workspace.subscriptionChanges[0].requestedPlanVersion.plan
                  .code,
              planName:
                item.workspace.subscriptionChanges[0].requestedPlanVersion.plan
                  .name,
        ...
        if (oldRenewalIds.length > 0) {
          await tx.paymentAttempt.updateMany({
            ...
            data: {
              ...
              completedAt: now,
              failureReason:
                "Renewal quote replaced by an approved billing change.",
            },
          });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
      ...
      .superRefine((value, context) => {
        if (
          ...
          context.addIssue({
            ...
            message:
              "Enterprise custom term requires the enterprise revenue model.",
          });
        }
    ...
    if (
      ...
      commitmentEndsAt =
        input.commitmentEndsAt ??
        getSubscriptionCommitmentBounds(startsAt, input.subscriptionTerm).end;
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (input.revenueModel === "PREPAID_PAYG") {
        await ensurePrepaidWalletInTransaction(tx, workspaceId, input.currency);
      }
        ...
        data: {
          ...
          metadata: {
            ...
            before: {
              ...
              commitmentEndsAt: before.commitmentEndsAt?.toISOString() ?? null,
            },
            after: {
              ...
              commitmentEndsAt: commitmentEndsAt?.toISOString() ?? null,
            },
            cancelledChanges,
            voidedInvoices,
          },
        ...
        if (invoiceIds.length > 0) {
          await tx.paymentAttempt.updateMany({
            ...
            data: {
              ...
              completedAt: now,
              failureReason:
                "Subscription request rejected by platform administrator.",
            },
          });
          await tx.billingInvoice.updateMany({
            where: { id: { in: invoiceIds } },
