...
function statusClass(status: string): string {
  ...
  if (["PAST_DUE", "SUSPENDED", "EXPIRED"].includes(status))
...
export default function AdminSubscriptionsPage() {
  ...
  async function submitOverride(
    ...
    try {
      ...
      setVariant("success");
      setMessage(
        "Subscription override applied and written to the audit trail.",
      );
      setOverride(null);
      await load();
    } catch (error) {
  ...
  async function reject(item: Subscription): Promise<void> {
    ...
    try {
      await apiRequest(
        ...
        {
          method: "POST",
          body: JSON.stringify({ note: "Rejected by platform administrator." }),
        },
      );
      setVariant("success");
  ...
  function updateOverride<K extends keyof OverrideState>(
    key: K,
    value: OverrideState[K],
  ): void {
    setOverride((current) =>
        ...
        <form className="card mb-4 border-primary" onSubmit={submitOverride}>
          <div className="card-header d-flex justify-content-between align-items-center gap-3">
            <div>
              ...
              <div className="small text-secondary">
                {override.subscription.workspace.name} ·{" "}
                {override.subscription.workspace.slug}
              </div>
            </div>
            <button
              className="btn-close"
          ...
          <div className="card-body">
            <div className="alert alert-warning py-2">
              This action applies immediately. Keep “cancel pending requests”
              enabled unless an existing payment or scheduled change must remain
              open.
            </div>
            <div className="row g-3">
              <div className="col-md-4">
                ...
                <select
                  ...
                  value={override.planCode}
                  onChange={(event) =>
                    updateOverride("planCode", event.target.value)
                  }
                >
              ...
              <div className="col-md-4">
                ...
                <select
                  ...
                  onChange={(event) => {
                    updateOverride(
                      "subscriptionTerm",
                      event.target.value as SubscriptionTerm,
                    );
                    updateOverride("periodEnd", "");
                    updateOverride("commitmentEndsAt", "");
                  }}
              ...
              <div className="col-md-3">
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateOverride("currency", event.target.value as Currency)
                  }
                >
              ...
              <div className="col-md-3">
                ...
                <input
                  ...
                  type="datetime-local"
                  value={override.periodStart}
                  onChange={(event) =>
                    updateOverride("periodStart", event.target.value)
                  }
                />
              </div>
              <div className="col-md-3">
                ...
                <input
                  className="form-control"
                  type="datetime-local"
                  value={override.periodEnd}
                  onChange={(event) =>
                    updateOverride("periodEnd", event.target.value)
                  }
                />
              </div>
              <div className="col-md-4">
                ...
                <input
                  className="form-control"
                  type="datetime-local"
                  value={override.commitmentEndsAt}
                  onChange={(event) =>
                    updateOverride("commitmentEndsAt", event.target.value)
                  }
                />
              </div>
              <div className="col-md-4">
                ...
                <input
                  className="form-control"
                  type="datetime-local"
                  value={override.trialEndsAt}
                  onChange={(event) =>
                    updateOverride("trialEndsAt", event.target.value)
                  }
                />
              </div>
              <div className="col-md-4">
                ...
                <input
                  className="form-control"
                  type="datetime-local"
                  value={override.graceEndsAt}
                  onChange={(event) =>
                    updateOverride("graceEndsAt", event.target.value)
                  }
                />
              </div>
              <div className="col-12">
                ...
                <textarea
                  ...
                  value={override.note}
                  onChange={(event) =>
                    updateOverride("note", event.target.value)
                  }
                />
              </div>
              <div className="col-md-6 form-check ms-2">
                <input
                  ...
                  onChange={(event) =>
                    updateOverride("cancelAtPeriodEnd", event.target.checked)
                  }
                />
                <label className="form-check-label" htmlFor="cancelAtPeriodEnd">
              ...
              <div className="col-md-5 form-check ms-2">
                <input
                  ...
                  type="checkbox"
                  checked={override.cancelPendingRequests}
                  onChange={(event) =>
                    updateOverride(
                ...
                <label
                  ...
                  htmlFor="cancelPendingRequests"
                >
                  Cancel pending plan requests and renewal quotes
                </label>
              </div>
            </div>
          </div>
          <div className="card-footer d-flex justify-content-end gap-2">
            ...
            <button
              ...
              disabled={busy === override.subscription.workspace.id}
            >
              {busy ? "Applying…" : "Apply manual override"}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            text="No workspaces match this status."
          />
        ) : (
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <strong>{item.plan.name}</strong> v{item.plan.version}
                      <div className="text-secondary small">
                        {item.currency} ·{" "}
                        {item.revenueModel.replaceAll("_", " ")} ·{" "}
                        {item.subscriptionTerm.replaceAll("_", " ")}
                      </div>
                      {item.commitmentEndsAt && (
                        <div className="text-secondary small">
                          Commitment to{" "}
                          {new Date(item.commitmentEndsAt).toLocaleDateString()}
                        </div>
                      )}
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      <div className="text-secondary small">
                        to {new Date(item.periodEnd).toLocaleDateString()}
                      </div>
                    </td>
                    <td>
                      {item.pendingChange ? (
                        <div>
                          <strong>{item.pendingChange.planName}</strong>
                          <div className="text-secondary small">
                            {item.pendingChange.currency} ·{" "}
                            {item.pendingChange.interval.toLowerCase()}
                          </div>
                          <div className="text-secondary small">
                            by {item.pendingChange.requestedBy.email}
                          </div>
                          <span
                            className={`badge mt-1 ${item.pendingChange.status === "APPROVED" ? "text-bg-info" : item.pendingChange.status === "PAYMENT_PENDING" ? "text-bg-primary" : "text-bg-warning"}`}
                          >
                            {item.pendingChange.status}
                          </span>
                        </div>
                      ) : (
                    ...
                    <td className="text-end">
                      <div className="d-flex justify-content-end flex-wrap gap-2">
                        {item.pendingChange?.status === "PENDING" && (
                          <>
                            <button
                              className="btn btn-sm btn-outline-success"
                              disabled={busy === item.workspace.id}
