            ...
            <div className="col-12">
              <div className="card">
                <div className="card-header d-flex flex-wrap justify-content-between gap-2">
                  ...
                  <span className={`badge ${data.imageOptimization.enabled ? "text-bg-success" : "text-bg-secondary"}`}>
                    {data.imageOptimization.enabled ? "enabled" : "disabled"}
                  </span>
                </div>
                <div className="card-body">
                  <div className="row g-3">
                    <div className="col-sm-6 col-xl-3"><div className="small text-secondary">Queued</div><div className="fs-4 fw-semibold">{data.imageOptimization.queued}</div></div>
                    <div className="col-sm-6 col-xl-3"><div className="small text-secondary">Active workers</div><div className="fs-4 fw-semibold">{data.imageOptimization.active} / {data.imageOptimization.concurrency}</div></div>
                    <div className="col-sm-6 col-xl-3"><div className="small text-secondary">Processed</div><div className="fs-4 fw-semibold">{data.imageOptimization.processed}</div></div>
                    <div className="col-sm-6 col-xl-3"><div className="small text-secondary">Failed</div><div className="fs-4 fw-semibold">{data.imageOptimization.failed}</div></div>
                  </div>
                  <div className="small text-secondary mt-3">
                    Output: {data.imageOptimization.outputFormat.toUpperCase()} · Sweep every {Math.round(data.imageOptimization.sweepIntervalMs / 1000)}s
                    {data.imageOptimization.lastProcessedAt ? ` · Last processed ${new Date(data.imageOptimization.lastProcessedAt).toLocaleString()}` : ""}
                  </div>
                  {data.imageOptimization.lastError && <div className="small text-danger mt-2">{data.imageOptimization.lastError}</div>}
                </div>
              </div>
            </div>
