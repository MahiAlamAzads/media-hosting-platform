        ...
        <>
          <div className="row g-3 mb-4">
            {[
              [
                "Active uploads",
                data.counts.activeUploads,
                "bi-cloud-arrow-up",
              ],
              ["Failed assets", data.counts.failedAssets, "bi-file-earmark-x"],
              [
                "Variant queue/failures",
                data.counts.queuedOrFailedVariants,
                "bi-images",
              ],
              ["Webhook failures", data.counts.webhookFailures, "bi-plug"],
              [
                "Usage email failures",
                data.counts.failedUsageEmails,
                "bi-envelope-exclamation",
              ],
              [
                "PAYG charge failures",
                data.counts.failedPaygCharges,
          ...
          <div className="row g-4">
            <div className="col-12">
              <div className="card">
                <div className="card-header d-flex flex-wrap justify-content-between gap-2">
                  <strong>Automatic image optimization</strong>
                  <span
                    className={`badge ${data.imageOptimization.enabled ? "text-bg-success" : "text-bg-secondary"}`}
                  >
                    {data.imageOptimization.enabled ? "enabled" : "disabled"}
                  </span>
                </div>
                <div className="card-body">
                  <div className="row g-3">
                    <div className="col-sm-6 col-xl-3">
                      <div className="small text-secondary">Queued</div>
                      <div className="fs-4 fw-semibold">
                        {data.imageOptimization.queued}
                      </div>
                    </div>
                    <div className="col-sm-6 col-xl-3">
                      <div className="small text-secondary">Active workers</div>
                      <div className="fs-4 fw-semibold">
                        {data.imageOptimization.active} /{" "}
                        {data.imageOptimization.concurrency}
                      </div>
                    </div>
                    <div className="col-sm-6 col-xl-3">
                      <div className="small text-secondary">Processed</div>
                      <div className="fs-4 fw-semibold">
                        {data.imageOptimization.processed}
                      </div>
                    </div>
                    <div className="col-sm-6 col-xl-3">
                      <div className="small text-secondary">Failed</div>
                      <div className="fs-4 fw-semibold">
                        {data.imageOptimization.failed}
                      </div>
                    </div>
                  </div>
                  <div className="small text-secondary mt-3">
                    Output: {data.imageOptimization.outputFormat.toUpperCase()}{" "}
                    · Sweep every{" "}
                    {Math.round(data.imageOptimization.sweepIntervalMs / 1000)}s
                    {data.imageOptimization.lastProcessedAt
                      ? ` · Last processed ${new Date(data.imageOptimization.lastProcessedAt).toLocaleString()}`
                      : ""}
                  </div>
                  {data.imageOptimization.lastError && (
                        ...
                        data.uploads.map((item) => (
                          <tr key={item.id}>
                            ...
                            <td>
                              ...
                              {item.receivedChunks}/{item.expectedChunks}
                            </td>
            ...
            <div className="col-xl-6">
              <div className="card h-100">
                ...
                <div className="list-group list-group-flush">
                  {data.failedAssets.length ? (
                    data.failedAssets.map((item) => (
                      <div className="list-group-item" key={item.id}>
                        <strong>{item.originalFilename}</strong>
                        <div className="small text-secondary">
                          {item.workspace.name} · {formatBytes(item.sizeBytes)}{" "}
            ...
            <div className="col-xl-6">
              <div className="card h-100">
                <div className="card-header">
                  <strong>Payment webhook failures</strong>
                </div>
                <div className="list-group list-group-flush">
            ...
            <div className="col-12">
              <div className="card">
                <div className="card-header">
                  <strong>PAYG card charge failures</strong>
                </div>
                <div className="table-responsive">
                  <table className="table mb-0">
                    <thead>
                    ...
                    <tbody>
                      {data.failedPaygCharges.length ? (
                        data.failedPaygCharges.map((item) => (
                          <tr key={item.id}>
                            <td>{item.workspace.name}</td>
                            <td>
                              {item.paymentMethod.provider} ·{" "}
                              {(
                                item.paymentMethod.brand ?? "CARD"
                              ).toUpperCase()}{" "}
            ...
            <div className="col-12">
              <div className="card">
                <div className="card-header">
                  <strong>Usage alert email failures</strong>
                </div>
                <div className="list-group list-group-flush">
                  {data.failedUsageEmails.length ? (
                    data.failedUsageEmails.map((item) => (
                      <div className="list-group-item" key={item.id}>
                        <div className="d-flex flex-wrap justify-content-between gap-2">
                          <strong>
                            {item.workspace.name} · {item.metric} ·{" "}
                            {item.threshold}%
                          </strong>
                          <span className="small text-secondary">
                        ...
                        <div className="small text-secondary">
                          {item.emailRecipient ?? "No recipient configured"}
                        </div>
                        <div className="small text-danger">
                          {item.emailLastError ?? "Unknown mail error"}
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="p-4 text-center text-secondary">
                      No usage alert email failures
                    </div>
                  )}
                </div>
              </div>
