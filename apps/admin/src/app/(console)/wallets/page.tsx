  ...
  async function adjust(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      if (!Number.isFinite(major) || major === 0) {
        throw new Error(
          "Enter a non-zero amount. Use a negative number to debit.",
        );
      }
      await apiRequest(
      ...
      {selected?.mode === "adjust" && (
        <form className="card mb-4 border-primary" onSubmit={adjust}>
          <div className="card-header d-flex justify-content-between align-items-center">
            <div>
              <strong>Adjust wallet balance</strong>
              <div className="small text-secondary">
                {selected.wallet.workspace.name}
              </div>
            </div>
            <button
              type="button"
          ...
          <div className="card-body">
            <div className="alert alert-info py-2">
              ...
              Balance cannot be reduced below reserved funds.
            </div>
            <div className="row g-3 align-items-end">
              <div className="col-md-3">
                <label className="form-label">
                  Amount ({selected.wallet.currency})
                </label>
                <input
      ...
      {selected?.mode === "settings" && (
        <form className="card mb-4 border-primary" onSubmit={saveSettings}>
            ...
            <div className="row g-3">
              <div className="col-md-3">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setSettingsStatus(event.target.value as WalletStatus)
                  }
                >
              ...
              <div className="col-md-3">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setSettingsCurrency(event.target.value as Currency)
                  }
                >
                ...
                <div className="form-text">
                  Currency changes only when balance and reserved funds are
                  zero.
                </div>
              </div>
              <div className="col-md-3">
                <label className="form-label">
                  Low-balance warning ({settingsCurrency})
                </label>
                <input
                  className="form-control"
                  inputMode="decimal"
          ...
          <div className="card-footer d-flex justify-content-end gap-2">
            ...
            <button className="btn btn-primary" disabled={busy}>
              {busy ? "Saving…" : "Save wallet settings"}
            </button>
          </div>
        </form>
      )}
        ...
        {loading ? (
          ...
          <div className="card-body">
            <EmptyState
              ...
              text="Wallets appear when a workspace is created or enters prepaid billing."
            />
          </div>
        ) : (
            ...
            <table className="table table-hover align-middle mb-0">
              <thead>
                <tr>
                  ...
                  <th className="text-end">Low warning</th>
                  <th>Updated</th>
              ...
              <tbody>
                {items.map((wallet) => (
                  <tr key={wallet.id}>
                    <td>
                      <strong>{wallet.workspace.name}</strong>
                      <div className="small text-secondary">
                        {wallet.workspace.slug}
                      </div>
                    </td>
                    <td>
                      {wallet.workspace.billingPreference?.revenueModel?.replaceAll(
                        "_",
                        " ",
                      ) ?? "—"}
                      <div className="small text-secondary">
                        {wallet.workspace.subscription?.planVersion.plan.name ??
                          "No plan"}
                      </div>
                    </td>
                    <td>
                      <span
                        className={`badge ${wallet.status === "ACTIVE" ? "text-bg-success" : wallet.status === "FROZEN" ? "text-bg-warning" : "text-bg-secondary"}`}
                      >
                        {wallet.status}
                      </span>
                    </td>
                    ...
                    <td className="text-end">
                      {formatMoneyMinor(wallet.reservedMinor, wallet.currency)}
                    </td>
                    <td className="text-end">
                      {formatMoneyMinor(
