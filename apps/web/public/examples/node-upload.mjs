...
if (!filePath) {
  console.error("Usage: node node-upload.mjs ./photo.jpg image/jpeg PUBLIC");
  process.exit(1);
}

const visibility =
  visibilityInput.toUpperCase() === "PRIVATE" ? "PRIVATE" : "PUBLIC";

const client = new MediaPlatformClient({
  baseUrl: process.env.MEDIA_PLATFORM_API_URL ?? "http://localhost:4000",
